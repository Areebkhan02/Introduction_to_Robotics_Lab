function error = PickPlace(pick, place)
...
%             end
        ...
        % In this state the end effector will be at z + 5, z coordinates.
            pick1_angles = findOptSolution([pick(1), pick(2), pick(3) + 5, pick(4)]);
            
            setPos(arb, pick1_angles);
            pause(5);
...
%             end
        ...
        elseif str_comp(stateVariable,'Pick2') 
            pick2_angles = findOptSolution([pick(1), pick(2), pick(3), pick(4)]); 
            
            setPos(arb, pick2_angles);
...
%             end
        ...
        elseif str_comp(stateVariable, 'Place1')
            place1_angles = findOptSolution([place(1), place(2), place(3) + 5, place(4)]);
            
            setPos(arb, place1_angles)
            pause(5);
...
%             end
        ...
        elseif str_comp(stateVariable, 'Place2')
            place2_angles = findOptSolution([place(1), place(2), place(3), place(4)]);
            
            setPos(arb, place2_angles)
            arb.setpos(5,0,64)
            pause(5);
            place2_angles = findOptSolution([place(1), place(2), place(3)+5, place(4)]);
